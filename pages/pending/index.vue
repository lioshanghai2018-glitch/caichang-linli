  ...
  <view class="popup-sheet" :class="{ show: showPopup }">
    ...
    <scroll-view class="popup-body" scroll-y>
      ...
      <view class="detail-section">
        ...
        <text class="detail-value price">￥{{ currentRefund.refundAmount || (currentRefund.order && currentRefund.order.payAmount) || 0 }}</text>
      </view>
      <view class="detail-section">
        <text class="detail-label">申请原因</text>
        <text class="detail-value">{{ currentRefund.reason || '-' }}</text>
      </view>
      <view class="detail-section" v-if="currentRefund.proofImages && currentRefund.proofImages.length > 0">
        <text class="detail-label">凭证图片</text>
        <view class="image-grid">
          <image v-for="(img, imgIdx) in currentRefund.proofImages" :key="imgIdx" class="proof-image" :src="img" mode="aspectFill" @tap="previewImage(img)" />
        </view>
      </view>
      <view class="detail-section">
        <text class="detail-label">退款金额调整</text>
        <view class="amount-input-row">
          <text class="amount-symbol">￥</text>
          <input class="amount-input" type="digit" v-model="refundAmount" />
        </view>
      </view>
      <view class="detail-section">
        <text class="detail-label">拒绝原因（拒绝时填写）</text>
        <textarea class="reject-input" v-model="rejectReason" placeholder="请输入拒绝原因，留空则使用默认原因" maxlength="200" />
      </view>
    </scroll-view>
  ...
  methods: {
    ...
    async fetchOrders() {
      ...
      try {
        ...
        if (this.orderList.length > this.prevOrderCount && this.prevOrderCount > 0) {
          this.playNewOrderSound()
        }
        this.prevOrderCount = this.orderList.length
    ...
    refundTypeLabel(type) {
      const map = { only_refund: '仅退款', refund_return: '退货退款', exchange: '换货' }
      return map[type] || type || '退款'
    },
    formatTime(t) {
      if (!t) return ''
      const d = new Date(t)
